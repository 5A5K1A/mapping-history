<?php
/**
 * Custom Post Type subclass Marker
 */
class Marker extends PostType {
	...
	public function GetHtml() {
		...
		if( has_post_thumbnail() ) {
			...
			$oThumb->caption = od_get_the_post_thumbnail_caption();
		}
	...
	/**
	 * Get marker data for ajax respons
	 */
	public function GetData() {
		...
		// set data
		$oData->title        = $this->Get('post_title');
		$oData->thumbnailUrl = $this->GetThumbnailUrl('marker_hover');
		$oData->photoYear    = get_field('image_year', $this->Get('ID'));
		$oData->lat          = $aLocation['lat'];
		$oData->lng          = $aLocation['lng'];
		$oData->url          = get_permalink($this->Get('ID'));
	 ...
	 */
	public function GetThumbnailUrl( $sThumbnailSize = 'fancybox' ) {
		// get thumbnail url
